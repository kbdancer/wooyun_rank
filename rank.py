...
def getVulInfo(url):
    ...
    try:
        ...
        try:
            ...
            if not cu.fetchone():
                cu.execute(
                    "INSERT INTO record (title,url,company,status,author,vultype,rank,sumittime,publishtime) VALUES (?,?,?,?,?,?,?,?,?)",
                    (title, url, company, status, author, vultype, rank, sumittime, publishtime))
                cx.commit()
...
if __name__ == '__main__':
    ...
    ifstart = raw_input(
        '[*] Public and confirm is ' + str(len(public_urllist) + len(confirm_urllist)) + '! Start now?(y/n):')
    if ifstart == 'y':
        bThread(checkList)
    else:
        sys.exit('[-] Exit!')